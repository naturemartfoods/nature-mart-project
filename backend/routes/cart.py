...
#     if not raw_image:
#         return ""
#     # Take only first image if comma-separated
...
#     # ✅ Each weight is a separate cart row
#     cur.execute(
...
#         # ✅ Pick correct price based on weight
...
def get_cart():
    ...
    for row in rows:
        ...
        cart_items.append({
            "cart_id":    cart_id,
            "id":         cart_id,       # ✅ keep "id" so old frontend still works
            "product_id": product_id,
            "name":       name,
            "price":      float(price),
            "image":      image_url,
...
# ✅ Keep old routes using product_id — so old frontend still works
@cart_bp.route('/cart/increase/<int:product_id>', methods=['PUT'])
@token_required
...
@cart_bp.route('/cart/decrease/<int:product_id>', methods=['PUT'])
@token_required
